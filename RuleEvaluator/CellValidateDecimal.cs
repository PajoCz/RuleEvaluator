...
{
    public class CellValidateDecimal : ICellValidate
    {
        public bool Validate(object p_CellFilter, object p_ValueDataForValidating)
        {
            ...
            if (!(p_CellFilter is CellValidateDecimalFilter)) throw new ArgumentException($"{nameof(p_CellFilter)} is not of expected type {typeof(CellValidateDecimalFilter)}");
            ...
            var filter = (CellValidateDecimalFilter) p_CellFilter;
